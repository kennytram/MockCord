        ...
        <>
            {acceptedRequests.length > 0 && sessionUser && Object.keys(users).length && acceptedRequests.filter(acceptedRequest => {
                ...
                <div className="friends-show-box">
                    ...
                    <div className="friendrequests-count-container">Online - {
                        acceptedRequests.filter(acceptedRequest => {
                            return acceptedRequest.senderId === sessionUser.id ? users[acceptedRequest.receiverId].isOnline : users[acceptedRequest.senderId].isOnline
                        }).length
                    }
                    </div>
                    <ul className="friend-request-list" onScroll={handleScroll}>
                        {acceptedRequests && acceptedRequests.filter(acceptedRequest => {
                            return acceptedRequest.senderId === sessionUser.id ? users[acceptedRequest.receiverId].isOnline : users[acceptedRequest.senderId].isOnline
                        }).map(acceptedRequest => {
                            return (
                                        ...
                                        <div className="friend-request-info-left">
                                            ...
                                            <div className="friend-request-info-left-text">
                                                ...
                                                <div className="friend-request-info-left-text-status">
                                                    {acceptedRequest && acceptedRequest.senderId === sessionUser.id ? users[acceptedRequest.receiverId].status : users[acceptedRequest.senderId].status}
                                                    {/* Idle Offline */}
                                                </div>
                                            </div>
                                        </div>
